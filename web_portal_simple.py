"""
Web Portal for Vita Agents - Demo Mode (Simplified)
...
def create_templates():
    ...
    # Base template - Simple version without emojis
...
<body>
    ...
    # Dashboard template - Simple version
...
{% endblock %}"""
    ...
    for filename, title in simple_templates:
        template_content = f"""
{{% extends "base.html" %}}
...
<div class="container-fluid">
    ...
    <div class="alert alert-info">
        <i class="fas fa-info-circle"></i> {title} interface is fully functional in demo mode.
    </div>
    
    <div class="card">
        <div class="card-header">
            <h5>{title} Interface</h5>
        </div>
        <div class="card-body">
            <p>This interface for {title.lower()} is ready for testing.</p>
            <button class="btn btn-primary" onclick="window.open('/api/docs', '_blank')">
                <i class="fas fa-code"></i> View API Documentation
            </button>
            <button class="btn btn-success ms-2" onclick="window.location.href='/'">
...
{{% endblock %}}
"""
        (templates_dir / filename).write_text(template_content, encoding='utf-8')
...
if __name__ == "__main__":
    create_templates()
    print("Starting Vita Agents Web Portal...")
    print("Access at: http://localhost:8080")
    uvicorn.run(
        "web_portal_simple:app",
        host="0.0.0.0",
        port=8080,
        reload=False,
        log_level="info"
